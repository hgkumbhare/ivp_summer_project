 ...
 * @author hgkumbhare
 * 
 * Has float image to buffered Image function
 * Has Buffered Image to Float Image function
 * Has writea image to a file function
 */
public class FloatToBufferedImage extends Configured implements Tool {
	...
	for (int i = 0; i < value.getHeight(); i++) {
		...
		int r = Math.round(dataBuff[3 * (i * value.getWidth() + j)]
			* (float) 255.0); // red component 0...255
		int g = Math.round(dataBuff[3 * (i * value.getWidth() + j) + 1]
			* (float) 255.0); // green component 0...255
		int b = Math.round(dataBuff[3 * (i * value.getWidth() + j) + 2]
			* (float) 255.0); // blue component 0...255
	...
	FloatImage floatImage = new FloatImage(bufferedImage.getWidth(),
		bufferedImage.getHeight(), 3, pixels);
	return floatImage;
    }

    public static void writeToAFile(FloatImage floatImage) {
